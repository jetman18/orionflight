...
void main_loop(){
	...
	roll_t.kp =3.9f;
	roll_t.ki =1.8f;//0.04f;
	roll_t.kd =0.7;//0.007f;//0.006f;
	roll_t.I =0;
	...
	while(1){
       ...
       FEQUENCY_DIV(2,ENABLE)
       {
    	   if(rx.ch[2]>MINIMUMTHROTLE){
            // pid calculate
    		pidCalculate(&pitch_t,mpu.pitch,Ch2,LOOPTIME);
			pidCalculate(&roll_t ,mpu.roll ,Ch1,LOOPTIME);
			pidCalculate(&yaw_t  ,mpu.yaw  ,Ch4,LOOPTIME);
